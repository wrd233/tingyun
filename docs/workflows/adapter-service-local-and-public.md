...
> 说明：本文保留更细的服务发布操作。当前仓库已经调整为共享 `knowledge/monitored_systems/` 和按批次落盘到 `artifacts/` 的结构；如与旧示例冲突，以仓库根 README 与 `docs/architecture/` 的现状说明为准。
...
- `knowledge_dir`
  ...
  - 默认建议使用仓库级共享目录 `../knowledge/monitored_systems`
- `service_host`
  - 本地启动监听地址
- `service_port`
  - 本地启动端口
