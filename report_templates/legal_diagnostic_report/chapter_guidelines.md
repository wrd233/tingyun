...
- `spec.yaml` 中的 `required_assets` / `optional_assets` 默认按 `diagnostics_root` 解析
...
- 占位文案应尽量稳定，优先使用：
  ...
  - `【暂无充分证据支撑该判断】`
  - `【待补充】`
- 排版骨架、章节顺序和固定样式优先由模板控制
- 章节中的对象筛选、数据摘要和自然语言组织由 agent / renderer 负责
...
- 不应在这一节臆造 diagnostics 中不存在的环境信息
- 系统名、批次、报告日期等实例级字段优先来自 `report_config.yaml`
...
- 如果 renderer 只拿到骨架上下文，可先输出模板占位说明
...
- 主机/端口信息如果仍主要来自样板，应明确标记为待实例化替换
- 这一章当前不允许从接口主表或 SQL 主表反推部署结论
...
- 慢 SQL 和异常 SQL 的表格列结构优先由模板固定，不要在实例层随意改列
- 若 `sql_master.csv` 中没有 `error_count > 0` 的条目，`2.3.3` 必须稳定输出占位，不得补造“异常 SQL”
...
- 缺 renderer 尚未支持的章节变量：允许先输出骨架式占位，不阻塞 `generated/` 结果落盘
- 连接池明细、主机清单、端口表如果 diagnostics 未直接沉淀，应在 `missing_data_report.md` 中同步记录为当前覆盖边界
